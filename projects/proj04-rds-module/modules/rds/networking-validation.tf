  ...
  lifecycle {
    ...
    postcondition {
      ...
      error_message = <<-EOT
        ...
        Please ensure that the subnet is property tagged by adding the following tags:
        1. Accces = "private"
        EOT
    }
  }
...
data "aws_vpc_security_group_rule" "input" {
  ...
  lifecycle {
    postcondition {
      ...
      1. Rules must not allow traffic from IP CIDR blocks, only from other security groups.
      EOT
    }
  }
}